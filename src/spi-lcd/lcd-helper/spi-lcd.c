...
#define SPI_DEVNAME   "/dev/spidev2.1"
...
int main(void) {
   ...
   /* Set up GPIO pins.
    * While sysfs is deprecated and will be removed fully at some point, it is
    * faster to integrate to the existing application as opposed to porting in
    * libgpiod. It also ends up being faster than used system() and gpioset
    * commands since the file handles are opened once at the start and remain
   ...
   write(fd, "240\n", 4); // LCD_RESET 240/5_80
   write(fd, "241\n", 4); // LCD_CMD 241/5_81
   write(fd, "238\n", 4); // Backlight 238/5_78
   close(fd);

   if ((fd = open("/sys/class/gpio/gpio240/direction",  O_RDWR|O_SYNC)) < 0) {
      fprintf(stderr, "Can't open /sys/class/gpio/gpio240/direction\n");
      return 1;
   }
   write(fd, "out\n", 4);
   close(fd);

   if ((fd = open("/sys/class/gpio/gpio241/direction",  O_RDWR|O_SYNC)) < 0) {
      fprintf(stderr, "Can't open /sys/class/gpio/gpio241/direction\n");
      return 1;
   }
   write(fd, "out\n", 4);
   close(fd);

   if ((fd = open("/sys/class/gpio/gpio238/direction",  O_RDWR|O_SYNC)) < 0) {
      fprintf(stderr, "Can't open /sys/class/gpio/gpio238/direction\n");
      return 1;
   }
   write(fd, "out\n", 4);
   close(fd);

   if ((lcdReset_fd = open("/sys/class/gpio/gpio240/value",  O_RDWR|O_SYNC)) < 0) {
   fprintf(stderr, "Can't open /sys/class/gpio/gpio240/value\n");
      return 1;
   }

   if ((lcdCmd_fd = open("/sys/class/gpio/gpio241/value",  O_RDWR|O_SYNC)) < 0) {
   fprintf(stderr, "Can't open /sys/class/gpio/gpio241/value\n");
      return 1;
   }

   if ((lcdBacklight_fd = open("/sys/class/gpio/gpio238/value",  O_RDWR|O_SYNC)) < 0) {
   fprintf(stderr, "Can't open /sys/class/gpio/gpio238/value\n");
      return 1;
   }
   ...
   while((ent = readdir(dir)) != NULL) {
      snprintf(name, sizeof(name), "/dev/%s", ent->d_name);
		...
		for (ret = readdir_r(dp, &entry, &ep); retVal == 0 && ret == 0 && ep
			...
			if (ep->d_type == DT_DIR) {
				...
				if (pidDir) {
					char buf[PATH_MAX];
					FILE *f;
					snprintf(buf, sizeof(buf), "/proc/%s/cmdline", ep->d_name);
					if ((f = fopen(buf, "r"))) {
